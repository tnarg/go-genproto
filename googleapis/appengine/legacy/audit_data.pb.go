...
func (m *AuditData) MarshalTo(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if len(m.EventMessage) > 0 {
		dAtA[i] = 0xa
		i++
		i = encodeVarintAuditData(dAtA, i, uint64(len(m.EventMessage)))
		i += copy(dAtA[i:], m.EventMessage)
	}
	if len(m.EventData) > 0 {
		for k, _ := range m.EventData {
			dAtA[i] = 0x12
			i++
			v := m.EventData[k]
			mapSize := 1 + len(k) + sovAuditData(uint64(len(k))) + 1 + len(v) + sovAuditData(uint64(len(v)))
			i = encodeVarintAuditData(dAtA, i, uint64(mapSize))
			dAtA[i] = 0xa
			i++
			i = encodeVarintAuditData(dAtA, i, uint64(len(k)))
			i += copy(dAtA[i:], k)
			dAtA[i] = 0x12
			i++
			i = encodeVarintAuditData(dAtA, i, uint64(len(v)))
			i += copy(dAtA[i:], v)
		}
	}
	return i, nil
...
func (m *AuditData) Size() (n int) {
	var l int
	_ = l
	l = len(m.EventMessage)
	if l > 0 {
		n += 1 + l + sovAuditData(uint64(l))
	}
	if len(m.EventData) > 0 {
		for k, v := range m.EventData {
			_ = k
			_ = v
			mapEntrySize := 1 + len(k) + sovAuditData(uint64(len(k))) + 1 + len(v) + sovAuditData(uint64(len(v)))
			n += mapEntrySize + 1 + sovAuditData(uint64(mapEntrySize))
		}
	}
	return n
}
...
func (m *AuditData) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			m.EventMessage = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
			...
			if m.EventData == nil {
				m.EventData = make(map[string]string)
			}
			var mapkey string
			var mapvalue string
			for iNdEx < postIndex {
...
var fileDescriptorAuditData = []byte{
	...
	0x0b, 0x46, 0xa7, 0xaa, 0x13, 0x8f, 0xe4, 0x18, 0x2f, 0x3c, 0x92, 0x63, 0x7c, 0xf0, 0x48, 0x8e,
	0x91, 0x4b, 0x3a, 0x39, 0x3f, 0x17, 0x97, 0x63, 0x9c, 0xf8, 0xe0, 0xae, 0x09, 0x00, 0x05, 0x4b,
	0x00, 0x63, 0x94, 0x2d, 0x54, 0x69, 0x7a, 0x7e, 0x4e, 0x62, 0x5e, 0xba, 0x5e, 0x7e, 0x51, 0xba,
	0x7e, 0x7a, 0x6a, 0x1e, 0x38, 0xd0, 0xf4, 0x21, 0x52, 0x89, 0x05, 0x99, 0xc5, 0x18, 0x21, 0x6c,
	0x0d, 0xa1, 0x92, 0xd8, 0xc0, 0x2a, 0x8d, 0x01, 0x01, 0x00, 0x00, 0xff, 0xff, 0x0b, 0xff, 0xde,
	0xb1, 0x8a, 0x01, 0x00, 0x00,
}
